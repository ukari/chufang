...
let
  ...
  stackConfigYamlPath = "${stackRoot}/config.yaml";
  stackConfigYaml = ''
...
in
{
  ...
  devcontainer.settings = {
    ...
    postCreateCommand = ''
      ...
      ${stackConfigYaml}
      EOF
      if [ ! -f ${stackConfigYamlPath} ]; then
        cp "$tmpfile" ${stackConfigYamlPath}
      else
        if [ "$(stat -c '%U:%G' ${stackConfigYamlPath})" != "${user}:${user}" ] ; then
          sudo chown ${user}:${user} ${stackConfigYamlPath}
        fi
        nix run nixpkgs#yq-go eval-all '. as $item ireduce ({}; . * $item)' ${stackConfigYamlPath} $tmpfile > "${stackConfigYamlPath}.tmp"
        mv "${stackConfigYamlPath}.tmp" "${stackConfigYamlPath}"
      fi
      rm $tmpfile
    '';
  };
